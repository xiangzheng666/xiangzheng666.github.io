...
from learning.model.selfatten import Self_attention
...
class RnnModel(nn.Module):
    ...
    def __init__(self, query_max_size, core_term_size, core_term_embedding_size,
        ...
        self.attenstion=Self_attention(30,30,30)
    ...
    def forward(self, matrix, length, core_terms):
        ...
        tmp=[]
        for i in torch.split(matrix, 1,dim=1):
            tmp.append(self.attenstion(i.squeeze(dim=1)).unsqueeze(dim=1))
        matrix=torch.cat(tmp,dim=1)
        ...
        x = torch.gather(x, 1, index.to(dtype=torch.int64))
        x = x[idx_unsort]
